# -*- coding: utf-8 -*-
"""
Kojto Finance Company Balance Line Model
"""
...
class KojtoFinanceBalanceCompanyLine(models.TransientModel):
    _name = "kojto.finance.balance.company.line"
    _description = "Company Balance Line"
    _order = "id"
    ...
    outgoing_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_comp_line_inv_out_rel', 'balance_line_id', 'invoice_content_id', string="Outgoing Pre-VAT Breakdown")
    incoming_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_comp_line_inv_in_rel', 'balance_line_id', 'invoice_content_id', string="Incoming Pre-VAT Breakdown")
    invoiceless_revenue_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_comp_line_cash_rev_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Revenue Breakdown")
    invoiceless_expenses_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_comp_line_cash_exp_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Expenses Breakdown")
    hr_time_tracking_breakdown = fields.Many2many('kojto.hr.time.tracking', 'bal_comp_line_tt_rel', 'balance_line_id', 'time_tracking_id', string="Time Tracking Breakdown")
    assets_works_breakdown = fields.Many2many('kojto.asset.works', 'bal_comp_line_aw_rel', 'balance_line_id', 'asset_work_id', string="Assets Works Breakdown")
    ...
    def action_view_breakdown(self):
        ...
        return {
            ...
            'name': 'Company Balance Breakdown',
            'res_model': self._name,
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('kojto_finance.view_kojto_finance_balance_company_line_form').id,
            'target': 'new',
        }
