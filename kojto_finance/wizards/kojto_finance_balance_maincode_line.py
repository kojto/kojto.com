# -*- coding: utf-8 -*-
"""
Kojto Finance Maincode Balance Line Model
"""
...
class KojtoFinanceBalanceMaincodeLine(models.TransientModel):
    _name = "kojto.finance.balance.maincode.line"
    _description = "Main Code Balance Line"
    _order = "maincode"
    ...
    maincode_id = fields.Many2one("kojto.commission.main.codes", string="Main Code")
    maincode = fields.Char(related="maincode_id.maincode", string="Main Code", readonly=True)
    description = fields.Char(related="maincode_id.description", string="Description", readonly=True)
    ...
    outgoing_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_maincode_line_inv_out_rel', 'balance_line_id', 'invoice_content_id', string="Outgoing Pre-VAT Breakdown")
    incoming_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_maincode_line_inv_in_rel', 'balance_line_id', 'invoice_content_id', string="Incoming Pre-VAT Breakdown")
    invoiceless_revenue_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_maincode_line_cash_rev_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Revenue Breakdown")
    invoiceless_expenses_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_maincode_line_cash_exp_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Expenses Breakdown")
    hr_time_tracking_breakdown = fields.Many2many('kojto.hr.time.tracking', 'bal_maincode_line_tt_rel', 'balance_line_id', 'time_tracking_id', string="Time Tracking Breakdown")
    assets_works_breakdown = fields.Many2many('kojto.asset.works', 'bal_maincode_line_aw_rel', 'balance_line_id', 'asset_work_id', string="Assets Works Breakdown")
    ...
    def action_view_breakdown(self):
        ...
        if self.date_from and self.date_to and self.maincode_id:
            ...
            breakdown = get_breakdown_records(
                ...
                maincode_ids=[self.maincode_id.id]
            )
        ...
        return {
            ...
            'name': 'Main Code Balance Breakdown',
            'res_model': self._name,
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('kojto_finance.view_kojto_finance_balance_maincode_line_form').id,
            'target': 'new',
        }
