# -*- coding: utf-8 -*-
"""
Kojto Finance Subcode Balance Line Model
"""
...
class KojtoFinanceBalanceSubcodeLine(models.TransientModel):
    _name = "kojto.finance.balance.subcode.line"
    _description = "Subcode Balance Line"
    _order = "subcode_name"
    ...
    maincode = fields.Char(related="subcode_id.maincode_id.maincode", string="Main Code", readonly=True)
    code = fields.Char(related="subcode_id.code_id.code", string="Code", readonly=True)
    subcode = fields.Char(related="subcode_id.subcode", string="Subcode", readonly=True)
    subcode_name = fields.Char(related="subcode_id.name", string="Subcode Name", readonly=True)
    description = fields.Char(related="subcode_id.description", string="Description", readonly=True)
    ...
    outgoing_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_subcode_line_inv_out_rel', 'balance_line_id', 'invoice_content_id', string="Outgoing Pre-VAT Breakdown")
    incoming_pre_vat_total_breakdown = fields.Many2many('kojto.finance.invoice.contents', 'bal_subcode_line_inv_in_rel', 'balance_line_id', 'invoice_content_id', string="Incoming Pre-VAT Breakdown")
    invoiceless_revenue_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_subcode_line_cash_rev_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Revenue Breakdown")
    invoiceless_expenses_breakdown = fields.Many2many('kojto.finance.cashflow.allocation', 'bal_subcode_line_cash_exp_rel', 'balance_line_id', 'allocation_id', string="Invoiceless Expenses Breakdown")
    hr_time_tracking_breakdown = fields.Many2many('kojto.hr.time.tracking', 'bal_subcode_line_tt_rel', 'balance_line_id', 'time_tracking_id', string="Time Tracking Breakdown")
    assets_works_breakdown = fields.Many2many('kojto.asset.works', 'bal_subcode_line_aw_rel', 'balance_line_id', 'asset_work_id', string="Assets Works Breakdown")
    ...
    def action_view_breakdown(self):
        ...
        if self.date_from and self.date_to and self.subcode_id:
            ...
            breakdown = get_breakdown_records(
                ...
                subcode_ids=[self.subcode_id.id]
            )
        ...
        return {
            ...
            'name': 'Subcode Balance Breakdown',
            'res_model': self._name,
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('kojto_finance.view_kojto_finance_balance_subcode_line_form').id,
            'target': 'new',
        }
