    ...
    def action_import(self):
        ...
        try:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start from 2 because we skipped header
                ...
                unit_weight = self._parse_number(unit_weight_str, "Unit Weight", row_num)
            ...
            if new_content_vals:
                created_contents = self.env["kojto.delivery.contents"].create(new_content_vals)

                # Recompute delivery content computed fields
                created_contents.compute_net_weight()
                created_contents._compute_quantity_package_contents()

                # Recompute delivery-level fields if any
                self.delivery_id.invalidate_recordset(['content'])
