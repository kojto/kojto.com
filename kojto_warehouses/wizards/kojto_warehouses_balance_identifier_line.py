# -*- coding: utf-8 -*-
"""
Kojto Warehouses Balance Identifier Line Model
"""
...
class KojtoWarehousesBalanceIdentifierLine(models.TransientModel):
    _name = "kojto.warehouses.balance.identifier.line"
    _description = "Warehouse Balance Identifier Line"
    _order = "identifier_id"
    ...
    identifier_name = fields.Char(string="Identifier Name", readonly=True, compute="_compute_identifier_info", store=True)
    identifier_type = fields.Selection(selection=[("material", "Material"), ("goods", "Goods"), ("asset", "Asset")], string="Identifier Type", readonly=True, compute="_compute_identifier_info", store=True)
    transaction_ids = fields.Many2many("kojto.warehouses.transactions", "wh_bal_id_tx_rel", "identifier_line_id", "transaction_id", string="Transactions", readonly=True)
    ...
    def _compute_identifier_info(self):
        """Compute identifier name and type from transactions"""
        # This is now computed in the wizard to avoid N+1 queries
        # Keep this method for backward compatibility but it should rarely be called
        for record in self:
            if record.identifier_name and record.identifier_type:
                continue  # Already set by wizard
            identifier_name = False
            identifier_type = False
            # Get identifier from first transaction that has one
    ...
    def _compute_totals(self):
        """Compute aggregated totals for this identifier - optimized single pass"""
        for record in self:
            transactions = record.transaction_ids
            if not transactions:
    ...
    def action_view_transactions(self):
        """Open a window showing all transactions for this identifier"""
        self.ensure_one()
        ...
        values = self.read(['identifier_id', 'wizard_id'])[0]
        ...
        return {
            'name': f'Transactions - Identifier {identifier_id} ({date_from} to {date_to})',
    ...
    def action_export_identifier_to_excel(self):
        """Export transactions for this identifier to Excel"""
        self.ensure_one()
        from .utils.kojto_warehouses_balance_wizard_actions import action_export_identifier_transactions_to_excel
        return action_export_identifier_transactions_to_excel(self.sudo())
