...
class KojtoOfferContentImportWizard(models.TransientModel):
    ...
    def action_import(self):
        ...
        try:
            ...
            if new_content_vals:
                created_contents = self.env["kojto.offer.contents"].create(new_content_vals)

                # Recompute offer content computed fields
                created_contents.compute_pre_vat_total()
                created_contents.compute_vat_total()
                created_contents.compute_total_price()

                # Recompute offer-level computed fields if any
                self.offer_id.invalidate_recordset(['content'])
